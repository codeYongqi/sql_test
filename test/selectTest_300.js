...
const selectTest = async function selectTest(time, thisCustomerBasedId) {
	let maxCost = -1 ;
	let minCost = 3600 ;
	let averageCost = 0;
	let accurency = 0;
    ...
    const rows = await query(`select uuid from customer_uuid where id = ${(thisCustomerBasedId% 6000)+1 }`);
    const customerId = rows[0].uuid;  

    for (let j = 0; j < count; j++) {

        const id = String((j * 6000) + (thisCustomerBasedId + 1)).padStart(8, 0);
        //const id = String((j * 6000) + 1);

		console.log(j)

        const rows2= await query(`select target from target where id = ${((j * 6000) % 500000) + (thisCustomerBasedId + 1) }`);
